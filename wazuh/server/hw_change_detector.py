        ...
        with open(ALERTS_FILE, 'r') as f:
            ...
            for line in f:
                try:
                    ...
                    # O'zgarishlar bo'lsa saqlash va alert yuborish
                    if changes:
...
def show_history(agent_id):
    ...
    hostname = row[0]
    print(f"\n{CYAN}{'='*60}{NC}")
    print(f"{BOLD}  {hostname} (Agent: {agent_id}) — HARDWARE TARIXI{NC}")
    print(f"{CYAN}{'='*60}{NC}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="HW-Detector: Hardware Change Detector — SQLite database boshqaruvi"
    )
